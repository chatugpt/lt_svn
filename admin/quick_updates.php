...
while (!$products->EOF) {
  ...
  if($prev_products_id != $products->fields['products_id']){
    ...
    if(QUICKUPDATES_DISPLAY_THUMBNAIL == 'true'){
      ...
      '<a href="javascript:selectFileID=\'SelectImageName_' . $products->fields['products_id'] . '\';updateImgId=\'SelectImageName_' . $products->fields['products_id'] . '_img\';popupWindow(\'' . zen_href_link(FILENAME_POPUP_FILE_SELECT, 'sdir=' . dirname((is_int(strpos($products->fields['products_image'],',')) ? substr($products->fields['products_image'],0,strpos($products->fields['products_image'],',')): $products->fields['products_image'] ) ) . '/') . '\');">' .
      zen_image(DIR_WS_CATALOG . DIR_WS_IMAGES . $products->fields['products_image'], TEXT_SELECT_IMAGE, QUICKUPDATES_DISPLAY_THUMBNAIL_WIDTH, QUICKUPDATES_DISPLAY_THUMBNAIL_HEIGHT, 'id="SelectImageName_' . $products->fields['products_id'] . '_img"') . '</a>'
       . '</td>' . "\n";
    }
    ...
    if(QUICKUPDATES_MODIFY_DESCRIPTION == 'true'||QUICKUPDATES_MODIFY_DESCRIPTION_POPUP == 'true') {
      ...
      //echo '<td class="smallText productsDescription">'
        echo '<a href="javascript:textEditFieldID=\'description_' . $products->fields['products_id'] . '\';popupWindow(\'' . zen_href_link(FILENAME_POPUP_TEXT_EDIT) . '\');">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', TEXT_HTML_EDIT_DESC) . '<span class="cssPopup">' . htmlspecialchars(stripslashes($products->fields['products_description'])) . '</span></a>';
      }
      echo "</td>\n";
    }
