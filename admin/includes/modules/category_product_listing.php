...
// show reset search
	...
	echo "<input name=cPath type=hidden value=0 />";
    echo '</form>';
	//---product id
	echo zen_draw_form('products_id', FILENAME_CATEGORIES, '', 'get');
// show reset search
    if (isset($_GET['products_id']) && zen_not_null($_GET['products_id'])) {
      echo '<a href="' . zen_href_link(FILENAME_CATEGORIES) . '">' . zen_image_button('button_reset.gif', IMAGE_RESET) . '</a>&nbsp;&nbsp;';
    }

    echo 'Product id :' . zen_draw_input_field('products_id') . zen_hide_session_id();
    if (isset($_GET['products_id']) && zen_not_null($_GET['products_id'])) {
      $keywords = zen_db_input(zen_db_prepare_input($_GET['products_id']));
      echo '<br/ >' . TEXT_INFO_SEARCH_DETAIL_FILTER . $keywords;
    }
	echo "<input name=cPath type=hidden value=0 />";
    echo '</form>';
...
<?php
    ...
    if (isset($_GET['products_id'])) {
      ...
      $categories = $db->Execute("select c.categories_id, cd.categories_name, cd.categories_description, c.categories_image,
                                  ...
                                  and cd.categories_id like '" . (int)zen_db_input($products_id) . "'" .
                                  $order_by);
...
?>
                ...
                <td class="dataTableContent" align="right">
                  <?php
				  $sqla = "select * from website where web_cid=".$categories->fields['categories_id'];
				  $rsa = mysql_query($sqla);
				  if(mysql_num_rows($rsa) == 0 && $_GET['cPath']>0 && isset($_GET['cPath'])){
					echo "<a href=product_acquisition_add.php?cID=".$categories->fields['categories_id'].">Not</a>";
				  }else{
				   echo '<a href="' . zen_href_link("product_acquisition_arr.php", 'cID=' . $categories->fields['categories_id'] ) . '&cPath='.$_GET['cPath'].'">' . zen_image(DIR_WS_IMAGES . 'icon_products_price_manager.gif', "update price") . '</a>'; 
				   }
				   ?>
...
<?php
    ...
    if (isset($_GET['products_id'])) {
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                ...
                                and pd.products_id = " . (int)zen_db_input($_GET['products_id']) . "
                                $order_by");
