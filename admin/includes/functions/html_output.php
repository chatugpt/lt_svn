...
//delete to images bof
  function delete_images($image_value){
  	$image_array = split(",",$image_value);
  	foreach ($image_array as $key => $value) {
	
		$a_value = substr($value,1);;
		$path .= "s".$a_value.",l".$a_value.",v".$a_value.",";
	}
	
	//delete file
	$image_path = split(",",$path);

	for ($k=0, $m=sizeof($image_path)-1; $k<$m; $k++) {
		$file_path = DIR_FS_CATALOG."images/".$image_path[$k];
		unlink($file_path);
	}
  }
  //delete to images eof	
?>