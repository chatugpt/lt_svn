...
function category_allowed($category_id){

// This function determines whether the category passed as parameter $category 
// should be displayed for the currently logged on user.

global $db;
   
  $who_allow = $db->Execute("select admin_id from ".TABLE_ADMIN_ALLOWED_CATEGORIES." where categories_id = '".$category_id."'");
    while (!$who_allow->EOF){
      if ($who_allow->fields['admin_id']==$_SESSION['admin_id']){return 'true';}
        $who_allow->MoveNext();
      }   
...
global $db;
  ...
  while (!$headers->EOF){ // for each valid menu header
	...
	// Read in list of all pages under this header from dB then for each page. Then for each page
	...
	// If hreaders.id is 21, we run the recusive function to display all categories with
	// subcategories.
	
	echo '<div class="checkboxBlock">'."\n";
	if ($headers->fields['id'] <> '21') {
    $query = "select * from ".TABLE_ADMIN_FILES." where header = '".$headers->fields['id']."'";
  	...
  	while (!$pages->EOF){
  		...
  		$query = "select * from ".TABLE_ADMIN_ALLOWED_PAGES." where page_id = '".$pages->fields['id']."' and admin_id = '".$adminID."'";
	...
	// BOF - Categories support
	  function draw_subcategories($adminID, $parent_id = 0, $ident = '') {
	    global $db;
      $header = $db->Execute("select header from ".TABLE_ADMIN_MENU_HEADERS." where id = '21'");
      ...
      $query = "select c.categories_id, cd.categories_name, c.parent_id
  	  ...
  	  while (!$categories->EOF){
   	    ...
   	    echo "\t".'<label>'.$ident.'<input name="'.'category_'.$categories->fields['categories_id'].'" class="'.$header.'" type="checkbox" ';
  		  $query = "select * from ".TABLE_ADMIN_ALLOWED_CATEGORIES." where categories_id = '".$categories->fields['categories_id']."' and admin_id = '".$adminID."'";
  		  $allow = $db->Execute($query);
  	 	  if ($allow->fields['admin_id']==$adminID) {echo 'checked="on" value="on">';}
  		  else {echo 'value="off">';}
  		  
        echo str_replace('_', ' ', $categories->fields['categories_name']);
          		
  		  echo "</label>\n<br/>";
  		  
        $new_ident = '&nbsp;&nbsp;&nbsp;'.$ident;
  		  draw_subcategories($adminID, $categories->fields['categories_id'],$new_ident);
  		  
  	  $categories->MoveNext();
  	  }
    }
    
    draw_subcategories($adminID);
    
  }
  echo "</div>\n\n";
	// EOF - Categories support
  $headers->MoveNext();
  }
}
?>