      ...
      if ($action == 'insert_category') {
        ...
        if (MODULE_ORDER_TOTAL_SC_ORDER_REWARD_POINTS == 'true') {
          if (isset($_POST['ratio'])) {
            ...
            $ratio = $_POST['ratio'];
            $sql = $db->Execute('INSERT INTO '.TABLE_SC_CATEGORIES_RATIO.' (categories_id, ratio) VALUES (\''.(int)$categories_id.'\', \''.$ratio.'\') ON DUPLICATE KEY UPDATE ratio=\''.$ratio.'\';');
          }
        }
        // end store credit
      }
      ...
      if ($_POST['categories_banner1_manual'] != '') {
        ...
        if ($categories_image = new upload('banner1_image')) {
          ...
          if ($categories_image->filename != 'none' && $categories_image->filename != '' && $_POST['image_delete'] != 1) {
            ...
            // remove filename when set to none and not blank
            if ($categories_image->filename != '' || $_POST['image_delete_banner1'] == 1) {
              $db->Execute("update " . TABLE_CATEGORIES . "
      ...
      if ($_POST['categories_banner2_manual'] != '') {
        ...
        if ($categories_image = new upload('banner2_image')) {
          ...
          if ($categories_image->filename != 'none' && $categories_image->filename != '' && $_POST['image_delete'] != 1) {
            ...
            // remove filename when set to none and not blank
            if ($categories_image->filename != '' || $_POST['image_delete_banner2'] == 1) {
              $db->Execute("update " . TABLE_CATEGORIES . "
...
//        $category_inputs_string .= '<INPUT type="hidden" name="categories_description[' . $languages[$i]['id']  . ']" ' . 'value=' . "'" . zen_get_category_description($cInfo->categories_id, $languages[$i]['id']) . "'>";
    ...
    if (MODULE_ORDER_TOTAL_SC_ORDER_REWARD_POINTS == 'true') {
      require_once(DIR_FS_CATALOG . 'includes/classes/store_credit.php');
      // get existing ratio
      $sql = 'SELECT ratio FROM '.TABLE_SC_CATEGORIES_RATIO.' WHERE categories_id='.$cInfo->categories_id.' LIMIT 1';
      $ratio = $db->Execute($sql);
      if($ratio->RecordCount() == 1){
        $ratio = $ratio->fields['ratio'];
      } else {
        $ratio = ''; // leave blank to disable
      }
      $contents[] = array('text' => '<br />' . TEXT_EDIT_SC_RATIO . '<br />' . zen_draw_input_field('ratio', $ratio));
    }
    // end store credit
