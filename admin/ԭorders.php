...
// ** GOOGLE CHECKOUT **
         ...
         if($num_rows != 0 && (strlen(htmlentities(strip_tags($notify_comments))) > GOOGLE_MESSAGE_LENGTH && MODULE_PAYMENT_GOOGLECHECKOUT_USE_CART_MESSAGING == 'True')) {
          ...
          $message = STORE_NAME . "\n" . EMAIL_SEPARATOR . "\n" .
          EMAIL_TEXT_ORDER_NUMBER . ' ' . zen_get_order_no($oID) . "\n\n" .
          EMAIL_TEXT_INVOICE_URL . ' ' . zen_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_no=' . zen_get_order_no($oID), 'SSL') . "\n\n" .
          ...
          $html_msg['EMAIL_TEXT_ORDER_NUMBER'] = EMAIL_TEXT_ORDER_NUMBER . ' ' . zen_get_order_no($oID);
          $html_msg['EMAIL_TEXT_INVOICE_URL']  = '<a href="' . zen_catalog_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_no=' . zen_get_order_no($oID), 'SSL') .'">'.str_replace(':','',EMAIL_TEXT_INVOICE_URL).'</a>';
  ...
  //send emails, not a google order or configured to use both messaging systems
	            zen_mail($check_status->fields['customers_name'], $check_status->fields['customers_email_address'], EMAIL_TEXT_SUBJECT . ' #' . zen_get_order_no($oID), $message, STORE_NAME, EMAIL_FROM, $html_msg, 'order_status');
	            $customer_notified = '1';
	//send extra emails
            }
            if (SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO_STATUS == '1' and SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO != '') {
              zen_mail('', SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO, SEND_EXTRA_ORDERS_STATUS_ADMIN_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . ' #' . zen_get_order_no($oID), $message, STORE_NAME, EMAIL_FROM, $html_msg, 'order_status_extra');
...
// ** END GOOGLE CHECKOUT **
	  ...
	  $_GET['action']= '';
		?>
		<form id="form1" name="form1" method="post" action="?page=<?php echo $_GET['page'];?>&oID=<?php echo $_GET['oID'];?>&action=deleteconfirm_a">
		  
		  <input type="text" name="pwd" />
		  <input type="submit" name="Submit" value="ok_pad" />
		</form>
		<?php 
		exit;
		break;
	  case 'deleteconfirm_a':
	  	$_GET['action']= '';
	  	if(isset($_POST['pwd']) && $_POST['pwd'] == $pwd){
			$_SESSION['password_a'] = $_POST['pwd'];
			zen_redirect('?page='.$_GET['page'].'&oID='.$_GET['oID'].'&action=deleteconfirm_ok');
			exit;	
		}else{
			zen_redirect(zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')), 'NONSSL'));
		}		
	  	break;
      case 'deleteconfirm_ok':
		// demo active test
		if($_SESSION['password_a'] == ""){
			zen_redirect(zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')), 'NONSSL'));
			exit;
		}
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main" valign="top"><strong><?php echo ENTRY_SHIPPING_ADDRESS; 
				?></strong></td>
...
<!-- BOF Admin order notes //--> 
<?php echo zen_draw_separator('pixel_trans.gif', '10', '1'); ?></td>
                <td valign="top">
<table width="100%" border="0" cellspacing="1" cellpadding="1">
                    <tr> 
                      <td><table width="75%" border="1" cellpadding="2" cellspacing="0">
                          <tr class="dataTableHeadingRow"> 
                          ...
                          <?php
    $admin_history = $db->Execute("select orders_id, date_added, comments from " . TABLE_ADMIN_COMMENTS . " where orders_id = '" . zen_db_input($oID) . "' order by date_added");
    if ($admin_history->RecordCount() > 0) {
      while (!$admin_history-> EOF) {
        echo '          <tr>' . "\n" .
...
?>
                    ...
                    <tr> 
                      <td><?php echo zen_draw_form('status', FILENAME_ORDERS, zen_get_all_get_params(array('action')) . 'action=update_admin_comments'); ?> 
                        <table width="75%" border="0" cellpadding="0" cellspacing="0" class="main">
                          <tr> 
                            <td class="main"><br><b>Add Confidential Admin Comments</b><br /></td>
                          </tr>
                          <tr> 
                            <td><?php echo zen_draw_textarea_field('admin_comments', 'soft', '60', '5'); ?></td>
                          </tr>
                          <tr> 
                            <td><?php echo zen_image_submit('button_update.gif', IMAGE_UPDATE); ?></td>
                          </tr>
                        </table></form></td>
                    </tr>
                  </table></td>
...
?>
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php if($_GET['oID'] != ""){echo "ID:".$_GET['oID'];}?></td>
            <td class="pageHeading" align="right"><?php echo zen_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
...
// create search_orders_products filter
  ...
  if (isset($_GET['search_orders_products']) && zen_not_null($_GET['search_orders_products'])) {
    ...
    $search = " and (op.products_model like '%" . $keywords . "%' or op.products_name like '%" . $keywords . "%' or op.products_id like '%" . $keywords . "%')";
...
/*  if(isset($_GET['oID']) && zen_not_null($_GET['oID'])){    //caizhouqing update
		$search = " and (o.order_no like '%" . $_GET['oID'] . "%')";
  }*/
...
//echo '<BR><BR>I SEE B: ' . $orders_query_raw . '<BR><BR>';
    ...
    }elseif ($_GET['search_orders_products'] != '') {
	
	//$rs=$db->Execute("select * from orders_products");
	//echo $rs->fields['products_name'];
		
      $orders_query_raw = "select * from (orders o,orders_products op) where o.orders_id = op.orders_id".$search." group by o.order_no";
...
?>
				...
				<?php echo '<a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders->fields['orders_id'] . '&action=deleteconfirm', 'NONSSL') . '" onclick="return ConfirmDel();">' . zen_image(DIR_WS_IMAGES . 'icon_delete.gif', ICON_EDIT) . '</a>'; ?>
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br /> <a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id, 'NONSSL') . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
...
// indicate if comments exist
      ...
      if (sizeof($order->products) > 0) {
        //$contents[] = array('align' => 'center', 'text' => '<a href="' . zen_hzen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit', 'NONSSL') . '">' . zen_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
