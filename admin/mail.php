...
?>
            <tr>
			  ...
			  if(isset($_GET['cID']) && $_GET['cID'] != ""){
			  	$rs = $db->Execute("select customers_email_address,customers_lastname,customers_firstname from customers where customers_id=".$_GET['cID']."");
				
			  	echo '<select rel="dropdown" name="customers_email_address"><option value="'.$rs->fields['customers_email_address'].'" SELECTED>'.$rs->fields['customers_lastname'].','.$rs->fields['customers_firstname'].' ('.$rs->fields['customers_email_address'].')</option></option></select>';
			  }else{
			  	echo zen_draw_pull_down_menu('customers_email_address', $customers, (isset($_GET['customer']) ? $_GET['customer'] : ''));  //, 'multiple' 
			  }
			  ?></td>
            </tr>
            <tr>
