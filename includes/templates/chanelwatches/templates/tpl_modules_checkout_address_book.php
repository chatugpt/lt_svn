 ...
 */
  ...
  //require('tpl_modules_address_book_details.php');
  //require('tpl_manager_address_default.php');
  $address = zen_get_address($_SESSION['customer_id']);  //»ñÈ¡µØÖ·
        ...
        <?php } ?>
	        ...
	        <ul class="pad_top">
	            ...
	            <a href="javascript:if(confirm('Are you sure deleting this shipping address?')){window.location.href='<?php echo zen_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS,'delete='.$addresses->fields['address_book_id']) ?>';}">
	              <?php 
				echo zen_image($template->get_template_dir('btn_delete.gif', DIR_WS_TEMPLATE, $current_page_base,'images/button'). '/btn_delete.gif')
				  ?>
	            </a>
				<?php }?>
	            </ul>
