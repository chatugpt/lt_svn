...
<?php }else{ ?>
<div class="bg_box_gray margin_t right_big_con" style="border:1px solid #E5E5E5;">
<?php } ?>
<h2 class="border_b" style="height:30px; line-height:30px; text-indent:1em; border:0;"><?php echo HEADING_TITLE; ?></h2>

<?php
	$faqs_all_array = array();
	if(empty($_GET['fcPath'])){
		if(intval($_GET['page'])==0 || intval($_GET['page'])==1){
			$from_number = 0;
		}else{
			$from_number = (intval($_GET['page'])-1)*MAX_DISPLAY_FAQS_ALL;
		}
	}
	$display_number = $from_number+MAX_DISPLAY_FAQS_ALL;
  ...
  $faqs_all_query_raw = "select f.*, fd.*, fcd.faq_categories_name from " . TABLE_FAQS . " f, " . TABLE_FAQS_DESCRIPTION . " fd, " . TABLE_FAQ_CATEGORIES_DESCRIPTION . " fcd, " . TABLE_FAQ_CATEGORIES . " fc " .
                        "where f.faqs_status = '1' and f.faqs_id = fd.faqs_id and fd.language_id = '" . (int)$_SESSION['languages_id'] . "'and fcd.language_id='" . (int)$_SESSION['languages_id'] . "' and f.master_faq_categories_id = fcd.faq_categories_id and f.master_faq_categories_id = fc.faq_categories_id and fc.faq_categories_status='1' and fc.faq_categories_id = '".$current_faq_category_id."'";
	}else{
  $faqs_all_query_raw = "select f.*, fd.*, fcd.faq_categories_name from " . TABLE_FAQS . " f, " . TABLE_FAQS_DESCRIPTION . " fd, " . TABLE_FAQ_CATEGORIES_DESCRIPTION . " fcd, " . TABLE_FAQ_CATEGORIES . " fc " .
                        "where f.faqs_status = '1' and f.faqs_id = fd.faqs_id and fd.language_id = '" . (int)$_SESSION['languages_id'] . "'and fcd.language_id='" . (int)$_SESSION['languages_id'] . "' and f.master_faq_categories_id = fcd.faq_categories_id and f.master_faq_categories_id = fc.faq_categories_id and fc.faq_categories_status='1' order by fc.sort_order, f.faqs_sort_order";
	}
  if(empty($_GET['fcPath'])){
        $tworow_query = "SELECT f.`faq_categories_id`, fd.`faq_categories_name`,fd.`language_id` FROM faq_categories f, faq_categories_description fd WHERE fd.faq_categories_id=f.faq_categories_id 
                   AND fd.language_id = '" . (int)$_SESSION['languages_id'] . "' ORDER BY f.`sort_order`  limit ".$from_number.",".$display_number."";
  //$faqs_all_split = new splitPageResults($faqs_all_query_raw,MAX_DISPLAY_FAQS_ALL);
  		$faqs_all_split = new splitPageResults($tworow_query,MAX_DISPLAY_FAQS_ALL);
  }else{
  		$faqs_all_split = new splitPageResults($faqs_all_query_raw,MAX_DISPLAY_FAQS_ALL);
  }
...
?>
  <div class="clear">
    <span class="fl" style="margin:0 0 5px 5px;"><?php echo $faqs_all_split->display_count(TEXT_DISPLAY_NUMBER_OF_FAQS_ALL); ?></span>
