...
?>
<?php if(!SimpleCache::StartBlock('tplindexcategories', true, false)): ?>
<div class="right_big_con margin_t">
<div class="midframe fl">

<?php
//the advertisement in home page
if($this_is_home_page) {
	if (SHOW_BANNERS_GROUP_SET1 != '' && $banner = zen_banner_exists('dynamic', SHOW_BANNERS_GROUP_SET1)) {
		if ($banner->RecordCount() > 0) {
			//require(DIR_WS_MODULES . 'sideboxes/'.$template_dir.'/flash_sidebox.php');
			echo "<div class=\"index_ad\">\n";
			$adstr = "<ul class=\"index_ad_img\">\n";
			$adtabstr = "<ul class=\"index_ad_btn\">\n";
			$i = 0;
			while(!$banner->EOF) {
				$adcls = $i === 0 ? ' class="first"' : '';
				$i++;
				$adstr .= '<li'.$adcls.'>'.zen_display_banner('static', $banner).'</li>'; 
				$adtabstr .= '<li'.$adcls.'>'. $i .'</li>';
				$banner->MoveNext();
			}
			$adstr .= '</ul>';
			$adtabstr .= '</ul>';
