...
//  echo 'I SEE ' . $this->total . ' vs ' . $this->free_shipping_price . ' items: ' . $this->free_shipping_item. '<br>';
		...
		if (in_array($key,array('BUST','WAIST','HIPS','HOLLOW_TO_HEM'))) {$this->total += strval(SIZE_SUBSIDIARY_PRICE);break;}
}
      } // attributes price
...
foreach((array)$this->contents[$products_id]['attributes'] as $key => $value) {
		if (in_array($key,array('BUST','WAIST','HIPS','HOLLOW_TO_HEM'))) {$attributes_price += strval(SIZE_SUBSIDIARY_PRICE);break;}
}
