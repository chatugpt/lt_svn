...
*/
	...
	class SEO_URL{
		...
		function SEO_URL($languages_id=''){
			...
			// Wordpress
			if (defined('FILENAME_WORDPRESS')) $seo_pages[] = FILENAME_WORDPRESS;
			...
			if ($this->not_null($parameters)) { 
				...
				switch ($page) {
					...
					case FILENAME_NEWS_INDEX:
						...
						break;
					case FILENAME_WORDPRESS:
						$link .= $this->make_url($page, FILENAME_NEWS_INDEX, 'wordpress', '', '', $separator);
						break;
...
//						case ($page == FILENAME_PRODUCT_INFO && !$this->is_attribute_string($params)):
						...
						case ($page == FILENAME_PRODUCT_FREE_SHIPPING_INFO):
							...
							break;
						case ($page == FILENAME_DOCUMENT_GENERAL_INFO):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'document_general_id', $p2[1], '.html', $separator);
							break;
						case ($page == FILENAME_DOCUMENT_PRODUCT_INFO):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'document_product_id', $p2[1], '.html', $separator);
							break;
