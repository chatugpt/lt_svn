      ...
      while (list(, $value) = each($this->modules)) {
		...
		//print_r($class);
          for ($i=0; $i<$size; $i++) {
            ...
            $output_string .= '<tr>'."\n".   
                              '<td align="right" width="380">' . $title . ' </td>' . "\n" .
                              '<td id="'.str_replace('_', '-', $GLOBALS[$class]->code).'" align="left">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                              '</tr>';
            }elseif(($GLOBALS[$class]->code == 'ot_total')){
			$output_string .= '<tr id="insurance_div" style="display:none">'."\n".
							  '<td align="right" width="380">Shipping Insurance:</td>' . "\n" .
							  '<td align="left" id="ot-insurance">'.$_SESSION['insurance']['price_str'].'</td>' . "\n" .
							  '</tr>'.
							  '<tr>'."\n".
                              '<td class="pad_b2" align="right" width="380"><h3>' . $GLOBALS[$class]->output[$i]['title'].'</h3></td>'. "\n" .
                              '<td align="left"><span id="'.str_replace('_', '-', $GLOBALS[$class]->code).'"><h3>' . $GLOBALS[$class]->output[$i]['text'] . '</h3></span></td>' . "\n" .
                              '</tr>';          	
            }
          }
