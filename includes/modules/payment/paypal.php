...
class paypal extends base {
  ...
  function process_button() {
	...
	$this->shipping_cost = ($order->info['shipping_cost']);  // * $currencies->get_value($my_currency)
    ...
    $optionsCore = array(
                   ...
                   'cancel_return' => zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'),
                   ...
                   'rm' => 2,
				   'bn' => 'Zhongtuo_Cart_EC_C2',
				   'mrb' => 'R-4DM17246PS436904F',
				   'pal' => 'GR5QUVVL9AFGN',
				   'os1' => $_SESSION['paypal_product_id'],
                   );
    ...
    $optionsShip = array(
                   ...
                   'no_shipping' => 0,//MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED caizhouqing
                   );
...
//      $optionsLineItems['num_cart_items'] = sizeof($order->products);
    ...
    $optionsAggregate = array(
                   'cmd' => '_ext-enter',
                   'item_name' => MODULE_PAYMENT_PAYPAL_PURCHASE_DESCRIPTION_TITLE ."#". $_SESSION['paypal_no'],//,$STORE_NAME
                   //'item_number' => MODULE_PAYMENT_PAYPAL_PURCHASE_DESCRIPTION_ITEMNUM,
                   //'num_cart_items' => sizeof($order->products),
                   'amount' => $paypal_amount_number,
                   'shipping' => $paypal_shipping_number,
...
//                   'paypal_order_id' => $paypal_order_id,
  ...
  function before_process() {
    ...
    if (isset($_GET['referer']) && $_GET['referer'] == 'paypal') {
      ...
      zen_redirect(zen_href_link(FILENAME_CHECKOUT_CONFIRMATION, '', 'SSL'));//caizhouqing update
    }
  }
  /**
    * Checks referrer
