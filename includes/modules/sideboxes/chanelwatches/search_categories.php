...
// don't build a tree when no categories
    if(isset($list_box_contents2)) {
          reset($list_box_contents2);
          $totalNum = sizeof($list_box_contents2);
    }
    for ($i = 0;$i < $totalNum; $i++){
		$rs = $db->Execute("select specials_new_products_price from specials where products_id=".$list_box_contents2[$i]['products_id']);
		if(!$rs->EOF){
		$priceListArray[] = $rs->fields['specials_new_products_price'];
		}else{	
