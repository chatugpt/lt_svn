...
if (EZPAGES_STATUS_FOOTER == '1' or (EZPAGES_STATUS_FOOTER == '2' and (strstr(EXCLUDE_ADMIN_IP_FOR_MAINTENANCE, $_SERVER['REMOTE_ADDR'])))) {
  ...
  $page_query = $db->Execute("select * from " . TABLE_EZPAGES . " where languages_id = {$_SESSION['languages_id']} AND status_footer = 1 and footer_sort_order > 0 order by footer_sort_order, pages_title");
