...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  $specials_index_query = "select p.products_id, p.products_image, pd.products_name, p.master_categories_id,s.specials_new_products_price,p.products_price
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $specials_index_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id,s.specials_new_products_price,p.products_price
                             from (" . TABLE_PRODUCTS . " p
                             left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id
                             left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id )
                             where p.products_id = s.products_id
...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
  	$products_price_tmp = $specials_index->fields['products_price'];      //caizhouqing update specials
	$specials_price_tmp = $specials_index->fields['specials_new_products_price'];
	//var_dump($products_price_tmp, $specials_price_tmp);
	if($products_price_tmp != ""){
		$specials_discount = 100-($specials_price_tmp / $products_price_tmp * 100);
	}
...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  $free_index_query = "select p.products_id, p.products_image, pd.products_name, p.master_categories_id,s.specials_new_products_price,p.products_price
  ...
  if (is_array($productsInCategory) && sizeof($productsInCategory) > 0) {
    ...
    $free_index_query = "select distinct p.products_id, p.products_image, pd.products_name, p.master_categories_id,s.specials_new_products_price,p.products_price
                             from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id," . TABLE_PRODUCTS_DESCRIPTION . " pd 
...
if ($num_products_count > 0) {
  ...
  while (!$free_index->EOF) {
  	$products_price_tmp = $free_index->fields['products_price'];      //caizhouqing update specials
	$specials_price_tmp = $free_index->fields['specials_new_products_price'];
	//var_dump($products_price_tmp, $specials_price_tmp);
	$discount_info = '';
	if($specials_price_tmp != ""){
		$specials_discount = 100-($specials_price_tmp / $products_price_tmp * 100);
		$discount_info = '<span class="sold_discount_sidebox2"><span class="sold_discount_font_sidebox2">'.round($specials_discount).'</span></span>';
	}
...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($new_products_category_id) || $new_products_category_id == '0') ) {
  $new_products_query = "select distinct p.products_id, p.products_image, p.products_tax_class_id, pd.products_name,
                                p.products_date_added, p.products_price, p.products_type, p.master_categories_id,s.specials_new_products_price,p.products_price
                           from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id," . TABLE_PRODUCTS_DESCRIPTION . " pd 
                           where p.products_id = pd.products_id
                           and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
...
if ($num_products_count > 0) {
  while (!$new_products->EOF) {
  	$products_price_tmp = $new_products->fields['products_price'];      //caizhouqing update specials
	$specials_price_tmp = $new_products->fields['specials_new_products_price'];
	//var_dump($products_price_tmp, $specials_price_tmp);
	$discount_info = '';
	if($specials_price_tmp != ""){
		$specials_discount = 100-($specials_price_tmp / $products_price_tmp * 100);
