 ...
 * @version $Id: fedexzones.php 6347 2007-05-20 19:46:59Z ajeh $
 */
/*
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_FEDEXZONES_METHOD) {
              ...
              if (ceil($shipping_weight*1000)/1000 <= $zones_table[$i]) {
                ...
                case (1):
                  ...
                  break;
                case (2):
                  $show_box_weight = ' (' . number_format($shipping_weight * $shipping_num_boxes,2) . MODULE_SHIPPING_FEDEXZONES_TEXT_UNITS . ')';
                  break;
                default:
                  $show_box_weight = ' (' . $shipping_num_boxes . ' x ' . number_format($shipping_weight,2) . MODULE_SHIPPING_FEDEXZONES_TEXT_UNITS . ')';
                  break;
                }

//                $shipping_method = MODULE_SHIPPING_FEDEXZONES_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_FEDEXZONES_TEXT_UNITS : '');
...
// shipping adjustment
        ...
        if ($shipping == -1) {
          ...
          switch (MODULE_SHIPPING_FEDEXZONES_METHOD) {
              ...
              $shipping_cost = ($shipping * 1) + constant('MODULE_SHIPPING_FEDEXZONES_HANDLING_' . $dest_zone);
              break;
        	  case (MODULE_SHIPPING_FEDEXZONES_METHOD == 'Price'):
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_FEDEXZONES_STATUS', 'MODULE_SHIPPING_FEDEXZONES_METHOD', 'MODULE_SHIPPING_FEDEXZONES_TAX_CLASS', 'MODULE_SHIPPING_FEDEXZONES_TAX_BASIS', 'MODULE_SHIPPING_FEDEXZONES_SORT_ORDER', 'MODULE_SHIPPING_FEDEXZONES_SKIPPED');
