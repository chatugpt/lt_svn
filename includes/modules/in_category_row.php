...
//		}
		$subCatoryString .= '</dl>';
		if($num_a > 5){
		$subCatoryString .= '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" class="more_product">More supply products ></a>';
		}
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<div><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" title="'.SEO_COMMON_KEYWORDS.' '.$in_categories->fields['categories_name'].'">' . zen_image(DIR_WS_IMAGES . $in_categories->fields['categories_image'], $in_categories->fields['categories_name'], 148, 113) . '</a></div><h3 class="margin_t line_120 pad_b2"><a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath='.$in_categories->fields['categories_id']) . '" title="'.SEO_COMMON_KEYWORDS.' '.$in_categories->fields['categories_name'].'">'. $in_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString);
