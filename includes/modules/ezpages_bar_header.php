...
if (EZPAGES_STATUS_HEADER == '1' or (EZPAGES_STATUS_HEADER == '2' and (strstr(EXCLUDE_ADMIN_IP_FOR_MAINTENANCE, $_SERVER['REMOTE_ADDR'])))) {
  ...
  $page_query = $db->Execute("select * from " . TABLE_EZPAGES . " where languages_id = {$_SESSION['languages_id']} AND  status_header = 1 and header_sort_order > 0 order by header_sort_order, pages_title");
