...
//  require(DIR_WS_FUNCTIONS . 'faqs.php');
  ...
  if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
    ...
    $process = true;
	  $faqs_contact_name = zen_db_prepare_input($_POST['fullname']);
	  $faqs_contact_mail = zen_db_prepare_input($_POST['sEmail']);
    $faqs_name = zen_db_prepare_input($_POST['faqs_name']);
    $faqs_category = zen_db_prepare_input($_POST['faqs_category']);
    $faqs_description = zen_db_prepare_input($_POST['faqs_description']);
	$faqs_type = '1';
...
// send additional emails
      if (SEND_FAQ_SUBMIT_EMAILS_TO_EMAILS_TO !='') {
       ...
       zen_mail('', SEND_FAQ_SUBMIT_EMAILS_TO_EMAILS_TO, SEND_EXTRA_SUBMIT_FAQ_TO_SUBJECT . ' ' . EMAIL_SUBJECT,
             $email_text . $extra_info['HTML'], STORE_NAME, EMAIL_FROM, $html_msg, 'contact_us');
      }
