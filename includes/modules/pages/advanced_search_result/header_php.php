...
$select_str = "SELECT DISTINCT " . $select_column_list .
              " m.manufacturers_id, p.products_id, pd.products_name, p.products_price, p.products_price_retail, p.product_is_wholesale, p.product_is_always_free_shipping,p.products_quantity_order_min, p.products_tax_class_id, p.products_price_sorter, p.products_qty_box_status, p.master_categories_id ";
...
//$search_keywords = array();    //����һ������

//$search_keywords = array('0' =>$_GET['keyword'] );
...
if (isset($keywords) && zen_not_null($keywords)) {
  if (zen_parse_search_string(stripslashes($_GET['keyword']), $search_keywords)) {
	...
	//$search_keywords[$i] = $_GET['keyword'];
	//echo $search_keywords[$i]."<br>";
      ...
      switch ($search_keywords[$i]) {
	    ...
	    //$where_str .= "(MATCH(p.products_model) AGAINST ('%:keywords%' IN BOOLEAN MODE)
        ...
        if (isset($_GET['search_in_description']) && ($_GET['search_in_description'] == '1')) {
         ...
         // $where_str2 .= " OR MATCH(pd.products_description) AGAINST ('%:keywords%' IN BOOLEAN MODE)";
          ...
          $where_str2 .= " OR (pd.products_description LIKE '%:keywords%')";
...
if (isset($_GET['dfrom']) && zen_not_null($_GET['dfrom']) && ($_GET['dfrom'] != DOB_FORMAT_STRING)) {
  ...
  $where_str2 .= " AND p.products_date_added >= :dateAdded";
  $where_str2 = $db->bindVars($where_str, ':dateAdded', zen_date_raw($dfrom), 'date');
}
...
if (isset($_GET['dto']) && zen_not_null($_GET['dto']) && ($_GET['dto'] != DOB_FORMAT_STRING)) {
  ...
  $where_str2 .= " and p.products_date_added <= :dateAdded";
  $where_str2 = $db->bindVars($where_str, ':dateAdded', zen_date_raw($dto), 'date');
}
if (DISPLAY_PRICE_WITH_TAX == 'true') {
  if ($pfrom) {
    ...
    $where_str = $db->bindVars($where_str, ':price', $pfrom, 'float');
     $where_str2 .= " AND (p.products_price_sorter * IF(gz.geo_zone_id IS null, 1, 1 + (tr.tax_rate / 100)) >= :price)";
    $where_str2 = $db->bindVars($where_str, ':price', $pfrom, 'float');
  }
  if ($pto) {
    $where_str .= " AND (p.products_price_sorter * IF(gz.geo_zone_id IS null, 1, 1 + (tr.tax_rate / 100)) <= :price)";
    $where_str = $db->bindVars($where_str, ':price', $pto, 'float');
    $where_str2 .= " AND (p.products_price_sorter * IF(gz.geo_zone_id IS null, 1, 1 + (tr.tax_rate / 100)) <= :price)";
    $where_str2 = $db->bindVars($where_str, ':price', $pto, 'float');
  }
} else {
  if ($pfrom) {
    $where_str .= " and (p.products_price_sorter >= :price)";
    $where_str2 .= " and (p.products_price_sorter >= :price)";
    $where_str = $db->bindVars($where_str, ':price', $pfrom, 'float');
    $where_str2 = $db->bindVars($where_str, ':price', $pfrom, 'float');
  }
  if ($pto) {
...
//caizhouqing update keyword bof
if(($_GET['main_page']=="advanced_search_result") && ($_GET['keyword'] !="") && (isset($_GET['keyword']))){
$products_keyword = addslashes(trim($_GET['keyword']));
$products_keyword_ip = $_SERVER["REMOTE_ADDR"];
$products_keyword_date = date("Y-m-d H:i:s",time());

	$sql="select * from products_keyword where products_keyword_title = '".$products_keyword."'";
	$rs = $db->Execute($sql);
	
	$current_time = strtotime($products_keyword_date) - strtotime($rs->fields['products_keyword_date']);
	
	if($rs->RecordCount() > 0){
		if($current_time > 600){
			$db->Execute("update products_keyword set products_keyword_hits=products_keyword_hits+1,products_keyword_date='".$products_keyword_date."',products_keyword_ip='".$products_keyword_ip."' where products_keyword_title = '".$products_keyword."'");
		}
	}else{
	$db->Execute("insert into products_keyword(products_keyword_title,products_keyword_date,products_keyword_ip) values ('".$products_keyword."','".$products_keyword_date."','".$products_keyword_ip."')");   //add
	}
}
//caizhouqing update keyword eof
