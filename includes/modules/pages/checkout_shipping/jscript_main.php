...
var subTotal =<?php echo ($showTotal * $currencies->get_value($_SESSION['currency']));?>;
var shippingPrice = 0;
var coupon = 0;
var reward=<?php echo $_SESSION['redeem_value']?$_SESSION['redeem_value']:0;?>;
...
var symbolRight='';
var insurancePrice=<?php echo $_SESSION['insurance']['price'];?>;
var discount_Price = 0;
var ot_categories_discount_price = 0;
var total = 0.00;
var currencies = "<?php echo $currencies->display_symbol_left($_SESSION['currency']);?>";
...
function priceCalculate(){
  ...
  var insurance = $_("insurance_checked");  
  ...
  for(i=0;i<shipping.length;i++){
    ...
    if(shipping[i].checked){
      ...
      rule = shipping[i].getAttribute("rule");
		  ...
		  //document.getElementById("Charges").innerHTML=currencies+" "+price;
...
function showPrice(){
  ...
  var coupon = $_("deduction").value;
  
  total = parseFloat(subTotal) + parseFloat(shippingPrice) - parseFloat(discount_Price) - parseFloat(coupon)-parseFloat(discount_group_price)-parseFloat(promo_discount) + parseFloat(insurancePrice)-parseFloat(reward);
 //alert(insurancePrice);
  if(insurancePrice > 0 ){
  	$_("insurance_div").style.display = '';
  }else{
  	$_("insurance_div").style.display = 'none';
  } 
...
function init(){
  ...
  if(shipping == null || shipping.length == 0){
    show('no_shipping');    
    hide('insurance_lab');
    $_('btn_place').disabled = true;
    $_('btn_place').src = '<?php echo $template->get_template_dir('btn_place_gray.gif', DIR_WS_TEMPLATE, $current_page_base,'images'). '/btn_place_gray.gif';?>';
  }
  else{
...
function check(fm){
  if(fm.display_coupon.checked == true){
	...
	//fm.action="index.php?apply=Redemption"
  }
  //made by liuyi2010-12-15
  var payment_n = fm.payment.length;
  var payment_flg = false;
	for(i=0;i<payment_n;i++){
		if(fm.payment[i].checked == true) {
			payment_flg = true;
			break;
		}
	}
	if (payment_flg == false) {
		alert("<?php echo TABLE_HEADING_PAYMENT_METHOD.' '.'information is required'; ?>");
		return false;
	}
