...
for ($i=0, $n=sizeof($products); $i<$n; $i++) {
  ...
  if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
    ...
    foreach ($products[$i]['attributes'] as $option => $value) {
      ...
      $attributes_options_name = isset($attributes_values->fields['products_options_name']) ? $attributes_values->fields['products_options_name'] : strtolower(str_replace('_',' ',$option));
      $attrArray[$option]['products_options_name'] = $attributes_options_name;
...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $buttonUpdate = ((SHOW_SHOPPING_CART_UPDATE == 1 or SHOW_SHOPPING_CART_UPDATE == 3) ? zen_image_submit(ICON_IMAGE_UPDATE, ICON_UPDATE_ALT) : '') . zen_draw_hidden_field('products_id[]', $products[$i]['id']);
	...
	}else{   //caizhouqing by eof zcar_products_Price
