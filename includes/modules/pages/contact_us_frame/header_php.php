...
$IMGVER_EnteredText=$_POST["txtCode"];
$IMGVER_RandomText=$_SESSION["IMGVER_RndText"];

if(strtolower($IMGVER_EnteredText) == strtolower($IMGVER_RandomText)){
  ...
  if ($zc_validate_email and !empty($enquiry) and !empty($name)) {
    ...
    if (empty($enquiry)) {
  	...
  	$err_msg = '&msg=Code error, please rewrite!';
  	zen_redirect(zen_href_link(FILENAME_CONTACT_US_FRAME, 'c=only'.$err_msg));
  }
} // end action==send
...
$define_page = zen_define_page(FILENAME_DEFINE_CONTACT_US_FRAME, false);
