...
switch ($_GET['main_page']) {
  ...
  //  case 'product_info':
  case (strstr($_GET['main_page'], 'product_') or strstr($_GET['main_page'], 'document_') or $_GET['main_page']=='print_page'):
  /*
  ...
  */
  ...
  if ($product_info_metatags->EOF) {
    ...
    if (!empty($product_info_metatags->fields['metatags_keywords']) or !empty($product_info_metatags->fields['metatags_description'])) {
      ...
      define('META_TAG_DESCRIPTION', str_replace('"','',TITLE . ' ' . $products_description . ' '));
      define('META_TAG_KEYWORDS', str_replace('"','',$meta_products_name . METATAGS_DIVIDER . "," . $breadcrumb->key_trail(",") ));

    } // CUSTOM META TAGS    KEYWORDS .
  } // EOF
  break;
