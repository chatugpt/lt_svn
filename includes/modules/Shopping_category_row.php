...
$title = BASE_COMMON_TEXT_SHOPPINGBYCATEGORIES;
...
$shopping_categories_query = "select c.categories_id,c.categories_image, cd.categories_name
										 ...
										 order by sort_order,cd.categories_name limit 12";
...
if ($num_categories > 0) {
  ...
  while (!$shopping_categories->EOF and !($row == 3)) {
    ...
    $parent_cPath_new = zen_get_path($shopping_categories->fields['categories_id']);

    // strip out 0_ from top level cats
    $parent_cPath_new = str_replace('=0_', '=', $parent_cPath_new);
...
WHERE c1.categories_id=c.categories_id and c.language_id = '.$_SESSION['languages_id'].' and parent_id = '.$shopping_categories->fields['categories_id'] .' limit 5';
		...
		$subCatoryString .= '<a href="' . zen_href_link(FILENAME_DEFAULT, $parent_cPath_new) . '" class="more_product">'.BASE_COMMON_TEXT_MOREWHOLESALEPRODUCTS.' ></a>';
		
    $list_box_contents[$row][$col] = array('params' => 'class="categoryListBoxContents"',
                                           'text' => '<a href="'. zen_href_link(FILENAME_DEFAULT, $parent_cPath_new) . '"  class="ih fl" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">' . zen_image(DIR_WS_IMAGES . $shopping_categories->fields['categories_image'], $shopping_categories->fields['categories_name'], 86, '',' class="fl"') . '</a><div class="block pad_l_90px"><h3><a href="' . zen_href_link(FILENAME_DEFAULT, $parent_cPath_new) . '" title="'.SEO_COMMON_KEYWORDS.' '.$shopping_categories->fields['categories_name'].'">'. $shopping_categories->fields['categories_name'] .'</a></h3>'.$subCatoryString.'</div>');
