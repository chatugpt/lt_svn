...
class ot_coupon {
  ...
  function collect_posts() {
	...
	}elseif (isset($_POST['insurance_checked']) && $_POST['insurance_checked'] == ""){
		$_SESSION['coupons']['insurance_checked']="";
	}
	...
	//exit;
  	...
  	$to_url="index.php?main_page=checkout_shipping";
    ...
    if (isset($_POST['dc_redeem_code']) && strtoupper($_POST['dc_redeem_code']) == 'REMOVE') {
      unset($_POST['dc_redeem_code']);
      unset($_SESSION['cc_id']);
      $messageStack->add_session('checkout_shipping', TEXT_REMOVE_REDEEM_COUPON, 'caution');
	  zen_redirect($to_url);
    }
    ...
    if ($_SESSION['cc_id'] > 0) {
      ...
      // remove if fails address validation
        if (!$foundvalid) {
        $messageStack->add_session('checkout_shipping', TEXT_REMOVE_REDEEM_COUPON_ZONE, 'caution');
        $this->clear_posts();
		zen_redirect($to_url);
    ...
    if ((isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] != '') || (isset($discount_coupon->fields['coupon_code']) && $discount_coupon->fields['coupon_code'] != '')) {
      ...
      if ($coupon_result->fields['coupon_type'] != 'G') {
		...
		//echo $coupon_result->fields['uses_per_user'];
		//exit;
	...
	// zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, 'credit_class_error_code=' . $this->code . '&credit_class_error=' . urlencode(TEST_NO_REDEEM_CODE), 'SSL', true, false));
	zen_redirect(zen_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL', true, false));
	exit;
	 }
  ...
  function get_order_total() {
	...
	//$orderTotalFull  ,  $order_total       $_POST['coupon_total']
