...
if (!defined('IS_ADMIN_FLAG')) {
  ...
  if (isset($_GET['min_price']) && isset($_GET['max_price']) && $_GET['min_price'] != '' && $_GET['max_price'] != ''){
  	...
  	$priceOrder = ' and  p.`products_price` >= "'.$_GET['min_price'].'" and p.`products_price` <= "'.$_GET['max_price'].'" ';  //caizhouqing update left_price
...
// We are asked to show only a specific category
      $listing_sql = "select " . $select_column_list . " p.products_id, p.products_type,p.products_status,p.products_price,p.products_quantity,p.product_is_wholesale, p.products_quantity_order_min, p.master_categories_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, pd.products_description, if(s.status = 1, s.specials_new_products_price, NULL) AS specials_new_products_price, IF(s.status = 1, s.specials_new_products_price, p.products_price) as final_price, p.products_sort_order, p.product_is_call, p.product_is_always_free_shipping, p.product_is_free, p.products_qty_box_status
