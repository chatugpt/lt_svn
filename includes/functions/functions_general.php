...
/**
* get define page by id
*/
function zen_define_page_content($title) {
	global $db;
	$title = $db->prepare_input($title);
	$sql = "SELECT `pages_html_text` FROM ".TABLE_EZPAGES." WHERE `pages_title` = '{$title}' AND `languages_id` = {$_SESSION['languages_id']}";
	$pages = $db->Execute($sql);
	return $pages->fields['pages_html_text'];
}

/**
* include the define page
*/
function zen_define_page($define_name, $require = true) {
	global $language_page_directory, $template_dir;
	if(substr($define_name, -4) != '.php') {
		$define_name .= '.php';
	}
	$defineDir = $language_page_directory . 'html_includes/';
	if(file_exists($language_page_directory . 'html_includes/' . $template_dir . '/' . $define_name)) {
		$defineDir = $language_page_directory . 'html_includes/' . $template_dir . '/';
	}
	//如果不直接包含，则返回简单页的路径。
	if(!$require) {
		return $defineDir . $define_name;
	}
	require $defineDir . $define_name;
}
/**
 ...
 */
  ...
  function zen_js_zone_list2($country, $form, $field) {
    global $db;
    $countries = $db->Execute("select distinct zone_country_id
                               from " . TABLE_ZONES . "
                               order by zone_country_id");
    ...
    while (!$countries->EOF) {
      ...
      $num_country++;
      $countries->MoveNext();
      $output_string .= '    show("zone_id_tr2");hide("zone_id_tr12");theForm.state.value="";' . "\n" ;
    }
    $output_string .= '  } else {' . "\n" .
                      '    hide("zone_id_tr2");show("zone_id_tr12");' . "\n" .
                      '  }' . "\n";
    return $output_string;
  }
