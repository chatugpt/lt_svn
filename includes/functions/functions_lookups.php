 ...
 */
  ...
  function zen_get_products_category_id_flash($products_id) {
    global $db;
    $the_products_category_query = "select products_id,master_categories_id as categories_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'" . " order by products_id,categories_id";
    $the_products_category = $db->Execute($the_products_category_query);
    return $the_products_category->fields['categories_id'];
  }
/*
 ...
 */
  function zen_get_products_image($product_id, $width = SMALL_IMAGE_WIDTH, $height = SMALL_IMAGE_HEIGHT) {    //caizhouqing update image
    global $db;

    $sql = "select p.products_image from " . TABLE_PRODUCTS . " p  where products_id='" . (int)$product_id . "'";
    $look_up = $db->Execute($sql);
	
	//echo DIR_WS_IMAGES . $look_up->fields['products_image'];
	//exit;
 ...
 */
  ...
  /*
 *  Look up whether a product is free
 */
  function zen_get_product_is_free($lookup) {
    global $db;

    $sql = "select p.product_is_free from " . TABLE_PRODUCTS . " p  where p.products_id='" . (int)$lookup . "'";
    $look_up = $db->Execute($sql);

    if ($look_up->fields['product_is_free'] == '1') {
      return true;
    } else {
      return false;
    }
...
*/
   function zen_get_freeshipping_same_products($products_name){
   	...
   	$string = '';
   	$query = "SELECT p.`products_id`,pd.`products_name` FROM ". TABLE_PRODUCTS." p , ". TABLE_PRODUCTS_DESCRIPTION." pd WHERE pd.`products_id`=p.`products_id` AND pd.`products_name` LIKE '" . zen_db_input($products_name) ."%free shipping' ";
