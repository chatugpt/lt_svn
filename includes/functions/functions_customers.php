...
// Return address  entry_street_address 
  function zen_get_address($country_id) {
    global $db;
    $address_format_query = "select entry_street_address
                             from address_book
                             where customers_id = '" . (int)$country_id . "'";

    $address_format = $db->Execute($address_format_query);

    if ($address_format->RecordCount() > 0) {
      return $address_format->fields['entry_street_address'];
    } else {
      return '';
    }
  }
